    ...
    public class AksUpdateOrchestrator : IAksUpdateOrchestrator
    {
        ...
        public async Task Run(ILogger log)
        {
            ...
            foreach (var notificationType in NotificationType.NotificationTypes)
            {
                ...
                await VerifyAksVersionChangesForLocations(supportedLocations, storedLatestVersionPerLocation, notificationType);
            }
        ...
        private async Task VerifyAksVersionChangesForLocations(IEnumerable<string> supportedLocations, IDictionary<string, Version> storedLatestVersionPerLocation, NotificationType notificationType)
        {
            foreach (var supportedLocation in supportedLocations)
            {
                await VerifyAksVersionChangesForLocation(supportedLocation, storedLatestVersionPerLocation, notificationType);
            }
        }

        public async Task VerifyAksVersionChangesForLocation(string supportedLocation, IDictionary<string, Version> storedLatestVersionPerLocation, NotificationType notificationType)
        {
